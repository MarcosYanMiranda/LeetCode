...
def countMatches(items, ruleKey, ruleValue):
    ...
    for i in range(len(items)):
        if ruleKey == 'type' and len(items[i][0]) == len(ruleValue):
            res += items[i][0].count(ruleValue) 
        elif ruleKey == 'color' and len(items[i][1]) == len(ruleValue):
            res += items[i][1].count(ruleValue)
        elif ruleKey == 'name' and len(items[i][2]) == len(ruleValue):
            res += items[i][2].count(ruleValue)
